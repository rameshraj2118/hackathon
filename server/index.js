...
const server = createServer(async (request, response) => {
  ...
  const entity = url.pathname.match(/^\/api\/(assets|departments|categories|employees|resources|bookings)(?:\/([^/]+))?$/)
  if (entity) { const auth = await authorizedUser(request); if (!auth?.user) return json(response, 401, { message: 'Your session has expired. Please sign in again.' }); const [_, table, id] = entity; const key = table === 'assets' ? 'tag' : table === 'departments' ? 'id' : table === 'categories' ? 'code' : table === 'employees' ? 'employee_id' : 'id'; if (request.method === 'GET') return json(response, 200, { [table]: listRows(table) }); if (request.method === 'POST' || request.method === 'PATCH') { const value = await body(request); if (!value || !value[key]) return json(response, 400, { message: `A ${key} is required.` }); saveRow(table, key, value); return json(response, 200, { item: value }) } if (request.method === 'DELETE' && id) { removeRow(table, key, id); return json(response, 204, {}) } }
