...
🛡️ Core Database & Authentication ArchitectureRelational Storage Engine: The application is powered by a high-performance SQLite database using the native better-sqlite3 driver. This ensures robust transaction logic, strict foreign-key relationships between tables (assets, users, bookings, audits), and proper data integrity for enterprise state transitions.  Cryptographic Security: Passwords are cryptographically hashed on the backend using Node.js's native scrypt algorithm.  Session Strategy: Local sessions leverage cryptographic signature verification via JSON Web Tokens (JWT).  Production Path: For production environments, the system can scale to a standalone database engine (e.g., PostgreSQL), coupled with HTTPS-only cookies, automated rate-limiting middleware, an integrated SMTP service, and a secure environment-injected JWT_SECRET.  
